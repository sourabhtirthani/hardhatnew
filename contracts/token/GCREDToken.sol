    ...
    AccessControlUpgradeable
{
    ...
    function initialize(address _MD_ADDRESS, address _DAO_ADDRESS) public initializer {
    ...
    function mintForReward(address to, uint256 amount) external onlyRole(STAKING_ROLE) {
        _mint(to, amount);
    }
    ...
    /// @inheritdoc IGCREDToken
    function bridgeBurn(address owner, uint256 amount) external onlyRole(BRIDGE_ROLE) {
        _burn(owner, amount);
    }

    /// @inheritdoc IGCREDToken
    function setStakingReward(address _STAKING_REWARD) external onlyRole(OWNER_ROLE) {
    ...
    function mint(address to, uint256 amount) external onlyRole(OWNER_ROLE) {
        _mint(to, amount);
    }
